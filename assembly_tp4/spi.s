...
.equ AT_FDCWD,   -100 // Flag que indica ao openat para usar o diretorio de trabalho atual como referencia relativa.
...
.equ SPI_IOC_WR_MODE,          0x40016B01 // Comando ioctl para definir o modo SPI (fase e polaridade do relogio - CPOL/CPHA).
.equ SPI_IOC_WR_BITS_PER_WORD, 0x40016B03 // Comando ioctl para definir o tamanho da palavra em bits.
.equ SPI_IOC_WR_MAX_SPEED_HZ,  0x40046B04 // Comando ioctl para definir a velocidade maxima de clock em Hz.
...
spi_is_open:
    ldr x0, =spi_fd
    ldr x0, [x0]
    cmp x0, #0 // Compara o valor com 0. No Linux, um File Descriptor valido e sempre > 0. Valores < 0 indicam erro.
...
spi_configure:
    ...
    ldr x1, =SPI_IOC_WR_MODE // Arg 2 do ioctl: Codigo do comando para alterar o modo.
    ldr x2, =spi_mode // Arg 3 do ioctl: Endereco da variavel contendo o valor do modo (0).
    bl  spi_ioctl

    ldr x1, =SPI_IOC_WR_BITS_PER_WORD // Codigo do comando para alterar os bits por palavra.
    ldr x2, =spi_bits
    bl  spi_ioctl

    ldr x1, =SPI_IOC_WR_MAX_SPEED_HZ // Codigo para alterar o clock do barramento.
    ldr x2, =spi_speed
    bl  spi_ioctl
...
spi_write_buf:
    ...
    ldr x2, =spi_fd // Pega o endereco de spi_fd.
    ldr x0, [x2] // Carrega o manipulador do arquivo em x0
    cmp x0, #0 // Verifica se o arquivo e valido.
    b.lt spi_write_fail
